...
let cmds = {
  ...
  repeat: { cmd: 'loop', a: ['lo','تكرار','اعادة'] },
  forceskip: { cmd: 'skip', a: ['s', 'fskip'] },
  ...
  if(serverQueue.repeating) {
	...
	dispatcher.setVolumeLogarithmic(serverQueue.volume / 100);
        ...
        if(!msg.member.hasPermission('SEND_MESSAGES')) {
          msg.react('❌')
          return msg.channel.send('You don\'t have permission `SEND_MESSAGES`');
        }
...
> ➤ Queue : عرض القائمة 
> ➤ loop : تكرار الاغنية 
> ➤ Skip او skipto : تخطي للاغنية التالية [s] او [st]
